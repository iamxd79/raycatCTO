...
export default function ArtworkImage({src,alt,preview,fill=false,width,height}:{src:string;alt:string;preview?:string;fill?:boolean;width?:number;height?:number}){
 ...
 return <div className={`museum-art-image ${fill?'is-focus':''}`} aria-busy={status==='loading'}>
  {preview&&<Image src={preview} alt="" fill aria-hidden="true" className="art-preview" loading="lazy"/>}
  <Image key={attempt} src={src} alt={alt} fill={fill} width={fill?undefined:width} height={fill?undefined:height} loading="lazy" onLoad={()=>setStatus('ready')} onError={()=>setStatus('error')} style={{opacity:status==='ready'?1:0}}/>
